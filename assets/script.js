$(document).ready(function(){
	...
	$(newGameBtn).click(function(){
		...
		setDogs(); // Set all the dog images
		timer = setInterval(setTime,1000); // Start the timer
	});
	// When click on PLAY AGAIN button
	$(playAgainBtn).click(function(){
		$("#winner").css("display","none"); // Hide completion message popup
...
let timer, gameCards, totalSeconds = 0, matchedPair = 0, clickedDogs = [], clickedDogsID = [];
// Declare all buttons
...
function setDogs(){
	...
	for(let i = 0; i < 4; i++)
	{
		for(let j = 0; j < 4; j++)
		{
			// Generate one random dog image from an array of dog images
			let randomDog = allDogs.splice(allDogs.length * Math.random() | 0, 1)[0];
			// Set the HTML div block for each dog image
			...
			// Reset all dog images array again once it becomes empty
			if(allDogs.length == 0)
				allDogs = ["dog1","dog2","dog3","dog4","dog5","dog6","dog7","dog8"];
		}
	}
	gameCards += '</div>';
	$("#game").html(gameCards); // Display all dog images
	alldogs(); // Set click events on each dog image box grid
}

// Show the dog image
function showDog(cid){
	// Check if dog images clicked 2nd time
	if(clickedDogs.length != 2)
	{
		...
		// Get the name of clicked dog image
		let dogName = document.querySelector("#back"+clickedID+" > img").src.split("/").pop().split(".")[0];
		clickedDogs.push(dogName); // Push the dog image name into an array of clicked dogs
		if(clickedDogs.length == 2) // Check if clickedDogs array length is equal to 2
		{
			...
			if(clickedDogs[0] == clickedDogs[1])
			{
				...
				if(matchedPair == 8) // Check if total matched pair is 8
				{
					clearInterval(timer); // Clear the timer countdown
					setTimeout(function(){
						$("#winner").css("display","block"); // Display the completion popup message
						$("#finalSeconds").text(totalSeconds);// Display total seconds taken to complete this game
					},800);
				}
				setTimeout(function(){
					// Set new background color of the matched dog images
					$("#back"+clickedDogsID[0]).css("background-color","#76db57");
					$("#back"+clickedDogsID[1]).css("background-color","#76db57");
					clickedDogs = []; // Empty the clicked dog image array
					clickedDogsID = []; // Empty the clicked dog ID array
				},500);
			}
			else // If both dog images are not matched
			{
				setTimeout(function(){
					...
					// Flip the front and back side of both clicked dog image box
...
// Set click event on every dog image
function alldogs(){
	let allCards = document.querySelectorAll(".card"); // Get all dog images
	for(let c = 0; c < allCards.length; c++)
	{
		// Set click event on each dog image and it will show the hidden image
...
function restartGame(){
	...
	allDogs = ["dog1","dog2","dog3","dog4","dog5","dog6","dog7","dog8"];
	setDogs(); // Set all dog image again
